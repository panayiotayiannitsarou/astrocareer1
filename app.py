...
_, lang_col=st.columns([6,1.25])
with lang_col:
    language=st.selectbox("Language",["Ελληνικά","English"],key="site_language",label_visibility="collapsed")
...
with tab1:
    ...
    if not chart:
        ...
        if st.button(t["replace"]): new_case(); st.rerun()
